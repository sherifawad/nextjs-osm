...
function usePlace() {
  ...
  const setLocationData = useCallback(
    ({
      ...
      try {
        const params = new URLSearchParams(searchParams);
        let latitude: number | undefined = parseFloat(
          searchParams.get("lat") ?? ""
        );
        let longitude: number | undefined = parseFloat(
        ...
        if (place) {
          ...
          if (place.rating.length > 0) {
            params.set(
              "count",
              `${2 * place._count.rating - place.rating.length}`
            );
            if (status === "authenticated") {
              const placeRate = place.rating.find(
