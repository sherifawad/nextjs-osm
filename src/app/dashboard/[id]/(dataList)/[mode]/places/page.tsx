...
async function PlacesPage({
  ...
  if (parsedSearchParams.success) {
    ...
    redirect(
      `http://${host}/dashboard/${session.user.id}/${pageParams.mode}/places?${params}`
    );
  }
        ...
        >
          <TabsList className="max-w-md mx-auto inline-flex xs:grid w-full grid-cols-2 mb-8">
            ...
            <TabsTrigger value="all-places">
              <Link
                href={`http://${host}/dashboard/${session.user.id}/all/places`}
              >
                All-Places
              </Link>
            </TabsTrigger>
